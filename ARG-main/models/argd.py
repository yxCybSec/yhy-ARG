...
class ARGDModel(torch.nn.Module):
    """
    蒸馏模型（ARG-D）：
    - teacher: 冻结的 ARGModel，用于提供教师特征 t_final_feature；
    - student: 只读取 content 的 BERT + Transformer + Attention + MLP。
    通过最小化 student 的 final_feature 与 teacher 的 final_feature 之间的 MSE，实现知识蒸馏。
    """
    def __init__(self, config):
        super(ARGDModel, self).__init__()

        # 教师模型：完整的 ARG 模型（在 Trainer 中会加载预训练好的权重）
        self.teacher = ARGModel(config)

        # 学生模型只保留一个 BERT，用于编码 content
    ...
    def forward(self, **kwargs):
        ...
        res = {
            ...
            's_final_feature': final_feature,      # 学生的最终特征
            't_final_feature': t_final_feature,    # 教师的最终特征（蒸馏目标）
            't_content_feature': t_content_feature,# 教师的内容特征（可用于进一步分析）  
        }
