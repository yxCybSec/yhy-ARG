...
class ARGModel(torch.nn.Module):
    ...
    主模型：同时编码
    - 新闻正文 content
    - 两种 LLM rationale（FTR_2, FTR_3）
    并通过 cross-attention / score reweight 等机制进行融合，完成假新闻二分类。
    """
    def __init__(self, config):
        super(ARGModel, self).__init__()

        # 两个 BERT：一个负责编码新闻正文，一个负责编码 LLM rationale
        ...
        # 只微调 BERT 的最后一层，其他层保持冻结，减少参数量和过拟合
        ...
        # 对 content + 各 expert 的最终表示做一次注意力聚合
        self.aggregator = MaskAttention(config['emb_dim'])
        # 主分类器 MLP：输入 final_feature，输出一个标量 logits
        self.mlp = MLP(config['emb_dim'], config['model']['mlp']['dims'], config['model']['mlp']['dropout'])

        # ---------- hard_ftr_*：rationale usefulness evaluator ----------
        # 使用 FTR 与 content 交互后的特征来预测“该 rationale 是否有用/是否正确”（二分类）
        ...
        # ---------- simple_ftr_*：LLM judgment predictor ----------
        # 使用仅来自 FTR 的信息，预测 LLM 的判断（real / fake / other，三分类）
        ...
        # 对 content 本身做 attention 聚合，得到一个单向的内容向量
        self.content_attention = MaskAttention(config['emb_dim'])    

        # Co-Attention 网络：建模 content 与 FTR 间的双向交互
        ...
        # Cross-Attention：从 content 出发、从 FTR 出发分别看对方
    ...
    def forward(self, **kwargs):
        """
        前向图的主要步骤：
        1. 分别用 BERT 编码 content / FTR_2 / FTR_3；
        2. 通过 cross-attention 得到三路 expert 表示；
        3. 对 expert 做 reweight，得到最终专家特征；
        4. 与 content 一起聚合成 final_feature，并输出 classify_pred；
        5. 同时输出 hard_ftr_* / simple_ftr_* 作为辅助任务。
        """
        ...
        # hard_ftr_*：预测“该条 rationale 是否有用/是否正确”
        ...
        # simple_ftr_*：仅基于 FTR 自身，预测 LLM 的分类决策（real/fake/other）
        ...
        # 将“FTR 视角看 content”的结果映射为 [0,1] 的权重，用于重加权 expert
        ...
        # 将正文向量 + 两个专家向量拼接，作为 aggregator 的输入
        ...
        res = {
            'classify_pred': torch.sigmoid(label_pred.squeeze(1)),  # [0,1] 的假新闻概率
            'gate_value': gate_value,                               # 两个专家的 gate 权重
            'final_feature': final_feature,                         # 用于蒸馏/分析的整体表示
